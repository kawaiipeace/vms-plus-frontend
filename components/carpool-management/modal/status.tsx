const STATUS_COLOR: Record<string, string> = {
  ...
  violet: "border-[#FED8F6] bg-[#FFF5FD] text-[#A80689]",
};

const STATUS_COLOR_GROUPS: Record<string, string[]> = {
  green: ["ปกติ", "เสร็จสิ้น", "ปฏิบัติงานปกติ"],
  orange: ["บำรุงรักษา", "รออนุมัติ", "ลาป่วย/ลากิจ", "ลา (ป่วย/กิจ)"],
  blue: ["ใช้ชั่วคราว", "ระหว่างโอน", "ค้างแรม", "ทดแทน", "ทดแทน (ปฏิบัติงานแทนคนอื่น, ย้ายมาจากสถานะสำรอง)"],
  violet: ["สำรอง"],
  red: ["ส่งซ่อม", "ไป - กลับ", "ให้ออก", "ให้ออก(BlackList)"],
...
export default function BadgeStatus({ status }: Readonly<BadgeStatusProps>) {
  ...
  return (
    <span className={`w-30 text-sm text-center font-bold px-3 py-1 rounded-full border ${className}`}>{status}</span>
  );
}
