...
import { fetchUserApprovalLic, fetchUserConfirmerLic } from "@/services/masterService";
...
>(({ valueFormStep1, requestData, driverData, editable, onTrackStatus, clearForm, onSubmit, onBack }, ref) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const basePayload: any = {
        ...
        driver_license_expire_date: convertToISO(String(valueFormStep1?.licenseExpiryDate), "00:00"),
      ...
      if (valueFormStep1?.driverLicenseType?.value === "2+" || valueFormStep1?.driverLicenseType?.value === "3+") {
        // Add certificate fields only if they exist
        if (valueFormStep1?.trainingEndDate) {
          basePayload.driver_certificate_expire_date = convertToISO(String(valueFormStep1.trainingEndDate), "00:00");
        }
        ...
        if (valueFormStep1?.trainingDate) {
          basePayload.driver_certificate_issue_date = convertToISO(String(valueFormStep1.trainingDate), "00:00");
        }
      ...
      if (editable) {
        console.log("next", driverData?.next_trn_request_annual_driver_uid);
        response = await resendLicenseAnnual(driverData?.next_trn_request_annual_driver_uid || "", basePayload);
        ...
        showToast({
          ...
          desc: (
            <>
              ...
              เลขที่ {response?.data?.result?.request_annual_driver_no} เรียบร้อยแล้ว
            </>
          ),
          seeDetail: {
            onClick: () => {
        ...
        <div className="modal-box max-w-[500px] p-0 relative overflow-hidden flex flex-col">
          <form>
            ...
            <div className={`modal-body overflow-y-auto text-left !bg-white`}>
              <p className="text-left text-base mb-2 font-semibold">Step 2: ผู้อนุมัติ</p>
              ...
              <div className="form-section">
                <div className="form-section-header">
                  <div className="form-section-header-title">ผู้อนุมัติให้ทำหน้าที่ขับรถยนต์</div>
                  {editable && (
                    <button
                      className="btn btn-tertiary-brand bg-transparent shadow-none border-none"
                      onClick={(e) => {
              ...
              <div className="text-left mt-5">
                ...
                >
                  ตามอนุมัติ ผวก. ลว. 16 ก.พ. 2541 เรื่อง ให้พนักงานของ กฟภ. ขับรถยนต์ที่ใช้ใบอนุญาตขับขี่ส่วนบุคคล
                </a>
              </div>
            </div>
