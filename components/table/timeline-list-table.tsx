...
const useColumns = (
                  ...
                  >
                    <div className={`flex items-center gap-1 text-sm font-semibold ${statusColors.text} py-[2px] px-[4px]`}>
                      ...
                      <div className="flex flex-col">
                        <span className="text-sm font-semibold">{item.schedule_title}</span>
                        <span className="text-black font-normal text-sm">{item.schedule_time}</span>
                      </div>
                    </div>
                  </div>
                ))}
  ...
  }, [columnHelper, dates, selectedOption, setDetailRequest, handleOpenDetailModal]);
};
...
export default function RequestListTable({ dataRequest, params, selectedOption, lastMonth }: RequestListTableProps) {
  ...
  const columns = useColumns(columnHelper, dates, selectedOption, lastMonth, setDetailRequest, handleOpenDetailModal);
  ...
  return (
    <div className="w-full py-4 pt-0 dataTable-bookingtimeline">
      ...
      <VehicleTimeLineDetailModal ref={vehicleTimelineDetailRef} detailRequest={detailRequest} />
    </div>
  );
}