...
import TripTimelineItem from "../vehicle-timeline/trip-timeline-item";
    ...
    const baseColumns = [
      columnHelper.accessor(
        ...
        }),
        {
          id: "licensePlate",
          header: () => (
            <span className="w-full">เลขทะเบียน / ยี่ห้อ / รุ่น</span>
          ),
          cell: (info) => {
            const { license, brandModel, brandName, provinceShort } =
              info.getValue();
            return (
              <div className="flex flex-col text-start">
                <span className="text-base font-semibold">
                  {license} {provinceShort}
                </span>
      ...
      selectedOption === "all"
        ? [
            ...
            columnHelper.accessor("distance", {
              header: () => (
                <span className="w-full">{`ระยะทาง ${lastMonth}`}</span>
              ),
              cell: (info) => (
                <div className="text-start">
                  <span>{info.getValue()}</span>
