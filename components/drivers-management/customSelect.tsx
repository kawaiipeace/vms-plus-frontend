        ...
        <ul className="max-h-[16rem] overflow-y-auto absolute flex drop-list-custom flex-col left-0 p-2 gap-2 z-10 mt-1 w-full border border-gray-300 rounded-lg shadow-lg">
          {options.length > 0 ? (
            options.map((option) => (
              <li
                key={option.value}
                className={`px-4 py-2 cursor-pointer flex flex-col gap-2 items-start rounded-lg ${
              ...
              >
                ...
                {option.labelDetail && <p className="text-sm text-gray-400">{option.labelDetail}</p>}
              </li>
            ))
          ) : (
