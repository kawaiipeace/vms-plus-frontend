...
const VehicleDetailModel = forwardRef<VehicleDetailModelRef, VehicleDetailModelProps>(
      ...
      <dialog ref={modalRef} className="modal">
        <div className="modal-box max-w-[1200px] p-0 relative bg-white">
          <div className="modal-header flex justify-between">
            ...
            <button className="btn btn-icon" onClick={() => modalRef.current?.close()}>
              ...
              <div className="modal-section">
                <div className="modal-section-inner">
                  <div className="modal-inner-title">
                    {vehicleDetail?.vehicle_brand_name?.trim()} {vehicleDetail?.vehicle_model_name?.trim()}
                  </div>
                  <div className="modal-inner-subtitle">{vehicleDetail?.vehicle_license_plate?.trim()}</div>
                  <div className="supporting-text-group">
                    <div className="supporting-text">{vehicleDetail?.CarType?.trim()}</div>
                    <div className="supporting-text">{vehicleDetail?.vehicle_owner_dept_sap?.trim()}</div>
                  </div>
                </div>
                ...
                <div className="card-item-group">
                  ...
                  <CarCardItem
                    ...
                    images={["/assets/img/ptt.png", "/assets/img/gas_3.svg", "/assets/img/gas_2.svg"]}
                    value={`${
                      vehicleDetail?.is_has_fleet_card ? vehicleDetail?.vehicle_department?.fleet_card_no : "ไม่มี"
                    }`}
                  />
                  <CarCardItem
                    icon="local_gas_station"
                    title="ประเภทเชื้อเพลิง"
                    value={vehicleDetail?.ref_fuel_type?.ref_fuel_type_name_th || "-"}
                  />
                  <CarCardItem
                    icon="airline_seat_recline_extra"
                    title="จำนวนที่นั่ง"
                    value={vehicleDetail?.seat ? `${vehicleDetail?.seat} ที่นั่ง` : "-"}
                  />
                  <CarCardItem
                    icon="auto_transmission"
                    title="ประเภทเกียร์"
                    value={vehicleDetail?.vehicle_gear === "NORMAL" ? "เกียร์ธรรมดา" : "เกียร์อัตโนมัติ"}
                  />
                  <CarCardItem
                  ...
                  <div className="form-card">
                    <div className="form-card-body form-card-inline items-center">
                      <div className="form-group form-plaintext form-users items-center">
                        <Image
                          src={vehicleDetail?.vehicle_department?.vehicle_user?.image_url || "/assets/img/avatar.svg"}
                        ...
                        <div className="form-plaintext-group align-self-center">
                          <div className="form-label">{vehicleDetail?.vehicle_department?.vehicle_user?.full_name}</div>
                          <div className="supporting-text-group">
                            <div className="supporting-text">
                              {vehicleDetail?.vehicle_department?.vehicle_user?.dept_sap}
                            </div>
                            <div className="supporting-text">
                              {vehicleDetail?.vehicle_department?.vehicle_user?.dept_sap_short}
                            </div>
                          </div>
                        </div>
                      </div>
                      <div className="form-card-right align-self-center">
                        <div className="flex flex-wrap gap-4">
                          {vehicleDetail?.vehicle_department?.vehicle_user?.tel_mobile && (
                          ...
                          {vehicleDetail?.vehicle_department?.vehicle_user?.tel_internal && (
                            <div className="col-span-12 md:col-span-6">
                              <div className="form-group form-plaintext">
          ...
          <div className="modal-action">
            ...
            {status !== "detail" && (
              <button
                ...
                onClick={() => {
                  if (onSelect) onSelect(vehicleDetail?.vehicle_brand_name?.trim() || "Unknown");
