...
const ReviewCarDriveModal = forwardRef<{ openModal: () => void; closeModal: () => void }, Props>(
  ({ displayOn, id }, ref) => {
    const modalRef = useRef<HTMLDialogElement>(null);
    const [reviewSubmit, setReviewSubmit] = useState(false);
    const [requestData, setRequestData] = useState<RequestDetailType>();
    const [ratting, setRatting] =
      useState<{ mas_satisfaction_survey_questions_code: string; survey_answer: number }[]>();
    const [satisfactionSurveyQuestions, setSatisfactionSurveyQuestions] = useState<SatisfactionSurveyQuestions[]>([]);
              ...
              <div className="flex gap-3 col-span-12">
                <div className="w-[60px] aspect-square rounded-2xl overflow-hidden">
                  <Image
                    ...
                    src={requestData?.driver?.driver_image || "/assets/img/sample-driver.png"}
                    width={100}
                    height={100}
                    alt=""
                  />
                ...
                <div className="text-left">
                  ...
                  <p className="text-xl">
                    ...
                    {requestData?.driver?.driver_nickname && `(${requestData?.driver?.driver_nickname})`}
                  </p>
                </div>
              </div>
