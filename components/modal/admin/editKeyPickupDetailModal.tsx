...
const KeyPickupDetailModal = forwardRef<{ openModal: () => void; closeModal: () => void }, Props>((props, ref) => {
  const { onSubmit, requestData } = props;

  const modalRef = useRef<HTMLDialogElement>(null);
  const [vehicleKeyTypeData, setVehicleKeyTypeData] = useState<any>([]);
  const [selectedAttach, setSelectedAttach] = useState<string>(requestData?.receiver_key_type?.toString() || "1");
  ...
  useEffect(() => {
    const fetchCostTypeFromCodeFunc = async () => {
      try {
        ...
        setSelectedAttach(
          requestData?.receiver_key_type?.toString() || response.data[0]?.ref_vehicle_key_type_code.toString()
        );
      } catch (error) {
          ...
          <div className="modal-box max-w-[500px] p-0 relative modal-vehicle-pick overflow-hidden flex flex-col max-h-[90vh]">
                    ...
                    <div className="col-span-6">
                      <div className="form-group">
                        ...
                        <div className="input-group">
                          ...
                          <TimePicker placeholder="ระบุเวลา" defaultValue={selectedTime} onChange={handleTimeChange} />
                        </div>
                      </div>
                    </div>
            ...
            <div className="modal-action sticky bottom-0 gap-3 mt-0">
              <div className="w-[50%] md:w-auto">
                <button className="btn btn-secondary w-full" onClick={handleCloseModal} disabled={isSubmitting}>
                  ไม่ใช่ตอนนี้
                </button>
              </div>
              <button className="btn btn-primary w-[50%] md:w-auto" onClick={submit} disabled={isSubmitting}>
