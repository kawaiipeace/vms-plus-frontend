...
const AdminDriverPickModal = forwardRef<{ openModal: () => void; closeModal: () => void }, Props>(
  ({ onSelect, onUpdate, reqId, onClickDetail }, ref) => {
    ...
    const handleSearch = (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      setSearchTerm(value);
      const filtered = drivers.filter((driver: DriverType) => driver.driver_name.includes(value));
    ...
    const [driverWorkTypeDatas, setdriverWorkTypeDatas] = useState<DriverWorkType[]>([]);
    ...
    const [selecteddriverWorkTypeOption, setSelecteddriverWorkTypeOption] = useState<{
      value: string;
      label: string;
    }>({ value: "", label: "ทั้งหมด" });

    const handledriverWorkTypeChange = async (selectedOption: CustomSelectOption) => {
      setSelecteddriverWorkTypeOption(selectedOption as { value: string; label: string });

      const data = driverWorkTypeDatas.find((work) => String(work.type) === String(selectedOption.value));
      if (data) {
        const filtered = drivers.filter((driver: DriverType) => String(driver.work_type) === selectedOption.value);
        setFilteredDrivers(filtered);
      }
    };
        ...
        try {
          ...
          if (response.status === 200) {
            ...
            const dataTypeArr = [
              ...
              ...workTypedata.map((cost: { type: string; description: string }) => ({
              ...
              {filteredDrivers.length > 0 && (
                <div className="relative w-full">
                  <div className="relative">
                    ...
                    >
                      <i className="material-symbols-outlined text-lg">keyboard_arrow_left</i>
                    </button>
                    ...
                    <button
                      ...
                      style={{ width: "40px", height: "40px" }}
                      onClick={() => setCurrentSlide((prev) => Math.min(prev + 1, groupedDrivers.length - 1))}
                      disabled={currentSlide === groupedDrivers.length - 1}
                    >
                      <i className="material-symbols-outlined text-lg">keyboard_arrow_right</i>
                    </button>
                        ...
                        {groupedDrivers[currentSlide]?.map((driver, index) => (
                          <div key={index} className="h-full">
                            <PickDriverCard
                              ...
                              driverStatus={driver.driver_status.ref_driver_status_desc || ""}
