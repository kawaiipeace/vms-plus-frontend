...
const DisbursementModal = forwardRef<{ openModal: () => void; closeModal: () => void }, Props>(
  ({ onUpdate, requestData, role }, ref) => {
    ...
    useEffect(() => {
      ...
      if (costTypeOptions.length > 0 && initialValues.refCostTypeCode) {
        const selectedOption = costTypeOptions.find(
          (option) => option.value === initialValues.refCostTypeCode
        );
    ...
    const onSubmit = async (data: any) => {
      if (requestData) {
        const payload = {
          cost_center: selectedCostTypeOption?.value === "2" ? selectedCostCenterOption?.value : data.costCenter,
        ...
        const updatedData = {
          ...
          costCenter: selectedCostTypeOption?.value === "2" ? selectedCostCenterOption?.value : data.costCenter,
        ...
        <form onSubmit={handleSubmit(onSubmit)}>
          ...
          <div className="modal-header bg-white sticky top-0 flex justify-between z-10">
            <div className="modal-title">แก้ไขการเบิกค่าใช้จ่าย</div>
              ...
              <button className="close btn btn-icon border-none bg-transparent shadow-none btn-tertiary" type="button" onClick={()=> modalRef.current?.close()}>
                  ...
                  <>
                    <div className="col-span-12">
                      <div className="form-group">
                        ...
                        <div className={`input-group ${errors.wbsNumber && "is-invalid"}`}>
                          <input
                            type="text"
                            className="form-control"
          ...
          <div className="modal-action sticky bottom-0 gap-3 mt-0">
                  ...
                  <button className="btn btn-secondary" type="button" onClick={()=>modalRef.current?.close()}>ยกเลิก</button>
