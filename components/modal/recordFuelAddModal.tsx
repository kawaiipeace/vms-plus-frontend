...
import { UserCreateAddFuelDetail, UserUpdateAddFuelDetail } from "@/services/vehicleInUseUser";
...
const RecordTravelAddModal = forwardRef<{ openModal: () => void; closeModal: () => void }, Props>(
  ({ status, isPayment, requestId, dataItem, role }, ref) => {
    ...
    useEffect(() => {
      if (status && dataItem) {
        const ref_fuel_type_id = fuelData.find((item) => item.ref_fuel_type_id === dataItem.ref_fuel_type_id);
          ...
          ref_oil_station_brand_id: {
            ...
            label: (
              <div className="flex items-center gap-1">
                ...
                <span className="ml-2">{ref_oil_station_brand_id?.ref_oil_station_brand_name_th}</span>
              </div>
            ),
          },
    ...
    const onSubmit = () => {
      ...
      const submitForm = async () => {
        try {
          ...
          if (status) {
            const res =
              role === "user" ? await UserUpdateAddFuelDetail(dataItem?.trn_add_fuel_uid || "", payload) : { data: {} };
            const data = res.data;
            if (data) {
          ...
          const res = role === "user" ? await UserCreateAddFuelDetail(payload) : await UserCreateAddFuelDetail(payload);
          const data = res.data;
          if (data) {
            modalRef.current?.close();
            ...
            //   ?
            router.push(
              pathName + `?activeTab=${activeTab}&create-fuel-req=success&tax_invoice_no=${data.data?.tax_invoice_no}`
            );
            // : router.push("/vehicle-booking/request-list?cancel-req=success&request-id=" + data.result?.request_no);
          }
        } catch (error) {
                ...
                <div className="col-span-12">
                  <div className="form-group">
                    ...
                    <CustomSelect
                      ...
                      onChange={(selected) => setValueForm((val) => ({ ...val, ref_oil_station_brand_id: selected }))}
                    />
                  </div>
                </div>
                <div className="col-span-12">
                  <div className="form-group">
                    ...
                    <CustomSelect
                      ...
                      onChange={(selected) => setValueForm((val) => ({ ...val, ref_fuel_type_id: selected }))}
                    />
                  </div>
                </div>
                  ...
                  <div className="">
                    <div className="form-group">
                      <label className="form-label">
                        เลขไมล์
                        <Tooltip title="เลขไมล์ขณะเติมเชื้อเพลิง" content="" position="right">
                      ...
                      <div className="input-group">
                        <input
                          ...
                          onChange={(e) => setValueForm((val) => ({ ...val, mile: e.target.value }))}
                        />
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="form-group">
                    ...
                    <div className="input-group">
                      <input
                        ...
                        onChange={(e) => setValueForm((val) => ({ ...val, tax_invoice_no: e.target.value }))}
                      />
                    </div>
                  </div>
                </div>
                    ...
                    <div className="form-group">
                      ...
                      <div className="input-group">
                        <input
                          ...
                          onChange={(e) => setValueForm((val) => ({ ...val, price_per_liter: e.target.value }))}
                        />
                      </div>
                    </div>
                  </div>
                    ...
                    <div className="form-group">
                      ...
                      <div className="input-group">
                        <input
                          ...
                          onChange={(e) => setValueForm((val) => ({ ...val, sum_liter: e.target.value }))}
                        />
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="form-group">
                    ...
                    <div className="input-group">
                      <input
                        ...
                        onChange={(e) => setValueForm((val) => ({ ...val, sum_price: e.target.value }))}
                      />
                    </div>
