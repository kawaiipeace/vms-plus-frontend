...
export default function SideBar({ menuName }: SidebarProps) {
  ...
  useEffect(() => {
    if (menuName) {
      ...
      } else if (["ตรวจสอบคำขอ", "อนุมัติใช้ยานพาหนะ", "ให้กุญแจและรับคืนยานพาหนะ"].includes(menuName)) {
        setOpenMenus(["collapseLink3"]);
      } else if (["ผู้ดูแลยานพาหนะ", "ข้อมูลพนักงานขับรถ", "ข้อมูลยานพาหนะ", "กลุ่มยานพาหนะ", "ข้อมูล Fleet card"].includes(menuName)) {
    ...
    >
      ...
      <div className={`flex items-center justify-between p-4 ${isExpanded && 'sidebar-top'}`}>
        <Link href="/" className="sidebar-brand">
            {isExpanded ? 
            <Image src={'/assets/img/brand.svg'} width={98} height={40} alt="" className="transition-all duration-300 ease-in-out" />
           :
            <Image src={'/assets/img/favicon.png'} width={40} height={40} alt="" className="transition-all duration-300 ease-in-out" />
          }
        </Link>
        ...
        <ul className="nav flex-col">
          <li className="nav-item">
            <Link href="/" className={`nav-link flex items-center ${activeItem === "home" ? "active" : ""}`}>
              ...
              <span className={`nav-link-label transition-all duration-300 ${isExpanded || isPinned ? "opacity-100 ml-2" : "opacity-0 w-0"}`}>
                หน้าหลัก
              </span>
            </Link>
          </li>
          ...
          {[
            {
              ...
              items: [{ title: "คำขอใช้ยานพาหนะ", link: "request-list" }, { title: "อนุมัติขอคำใช้และใบอนุญาต", link: "request-list" }],
            },
            {
              id: "collapseLink3",
              icon: "traffic_jam",
            ...
            <li className={`nav-item  ${!isExpanded && 'h-[40px] w-[40px]'}`} key={menu.id}>
              <button
                ...
                className={`nav-link flex items-center justify-between ${!isExpanded && 'h-[40px] w-[40px]'} ${openMenus.includes(menu.id) ? "active" : ""}`}
              >
                <div className="flex items-center">
                  <i className="material-symbols-outlined">{menu.icon}</i>
                  <span className={`nav-link-label text-left transition-all duration-300 ${isExpanded || isPinned ? "opacity-100 ml-2" : "opacity-0 w-0"}`}>
                    {menu.label}
                  </span>
                </div>
                {isExpanded &&
                <i className="ico-toggle">
                  <span className="material-symbols-outlined">{openMenus.includes(menu.id) ? "keyboard_arrow_up" : "keyboard_arrow_down"}</span>
                </i>
                 }
              </button>
              {isExpanded &&
              <div className={`transition-all duration-500 ease-in-out overflow-hidden ${openMenus.includes(menu.id) ? "max-h-[500px] block" : "hidden"}`}>
                <ul className="nav flex-col text-left">
                  {menu.items.map((item, index) => (
                    <li className="nav-item text-left" key={index}>
                      <Link href={item.link} className={`nav-link flex items-center ${activeItem === item.title ? "active" : ""}`}>
                        <span className={`nav-link-label text-left transition-all duration-300 ${isExpanded || isPinned ? "block" : "hidden"}`}>
                          {item.title}
                        </span>
                      </Link>
                    </li>
